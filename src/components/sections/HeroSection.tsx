...
export function HeroSection() {
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center px-6 bg-[var(--bg)] overflow-hidden pt-14">
      {/* Particle Background */}
      <ParticleBackground className="absolute inset-0 z-0 w-full h-full" />

      {/* Overlay gradient for better text readability */}
      <div className="absolute inset-0 z-[1] bg-gradient-to-b from-[var(--bg)]/60 via-transparent to-[var(--bg)]" />
      ...
      <div className="relative z-10 flex flex-col items-center justify-center text-center max-w-3xl mx-auto gap-8">
        ...
        >
          <Link
            href="/signup"
            className="h-11 px-6 rounded-lg bg-[var(--accent)] hover:bg-[var(--accent-light)] text-white text-sm font-medium transition-all duration-200 flex items-center gap-2 shadow-[0_0_24px_rgba(242,203,56,0.25)] hover:shadow-[0_0_32px_rgba(242,203,56,0.35)]"
          >
          ...
          >
            <Play size={14} className="text-[var(--accent-light)]" />
            Voir une démo
          </Link>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-2">
            <div className="flex -space-x-2">
              {["J", "M", "S", "T", "N"].map((initial, i) => (
                <div
                  key={i}
                  className="w-7 h-7 rounded-full bg-gradient-to-br from-accent to-[var(--accent-light)] flex items-center justify-center text-[10px] font-bold text-white border-2 border-[var(--bg)]"
                >
                  {initial}
                </div>
              ))}
          ...
          <div className="flex items-center gap-2">
            <svg className="w-4 h-4 text-[var(--accent-light)]" fill="currentColor" viewBox="0 0 20 20">
        ...
        >
          ...
          <div className="text-center">
            <p className="text-xl sm:text-2xl font-bold text-[var(--accent-light)]">70%</p>
